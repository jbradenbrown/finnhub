...
impl<'a> MutualFundEndpoints<'a> {
    ...
    pub async fn profile(&self, symbol: Option<&str>, isin: Option<&str>) -> Result<MutualFundProfile> {
        let mut params = vec![];
...
mod tests {
    ...
    async fn test_profile() {
        ...
        assert!(result.is_ok(), "Failed to get mutual fund profile: {:?}", result.err());
    }
    ...
    async fn test_holdings() {
        let client = test_client().await;
        let result = client.mutual_fund().holdings(Some("VTSAX"), None, None).await;
        assert!(result.is_ok(), "Failed to get mutual fund holdings: {:?}", result.err());
    }
    ...
    async fn test_country_exposure() {
        let client = test_client().await;
        let result = client.mutual_fund().country_exposure(Some("VTSAX"), None).await;
    ...
    async fn test_sector_exposure() {
        let client = test_client().await;
        let result = client.mutual_fund().sector_exposure(Some("VTSAX"), None).await;
