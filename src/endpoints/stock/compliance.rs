...
use crate::{
    ...
    error::Result,
    models::stock::{
        CompanyExecutives, CongressionalTrading, ESGScore, Lobbying, SupplyChainData,
        USASpending, USPTOPatents, VisaApplications,
    },
};
...
mod tests {
    ...
    async fn test_executives() {
        ...
        assert!(result.is_ok(), "Failed to get executives: {:?}", result.err());
    }
    ...
    async fn test_congressional_trading() {
        let client = test_client().await;
        let result = client.stock().congressional_trading("AAPL", None, None).await;
        
        // Congressional trading data might not be available for all companies
        assert!(result.is_ok(), "Failed to get congressional trading: {:?}", result.err());
    }
    ...
    async fn test_usa_spending() {
        ...
        assert!(result.is_ok(), "Failed to get USA spending: {:?}", result.err());
    }
    ...
    async fn test_supply_chain() {
        ...
        assert!(result.is_ok(), "Failed to get supply chain: {:?}", result.err());
    }
    ...
    async fn test_uspto_patents() {
        ...
        assert!(result.is_ok(), "Failed to get USPTO patents: {:?}", result.err());
    }
    ...
    async fn test_visa_applications() {
        ...
        assert!(result.is_ok(), "Failed to get visa applications: {:?}", result.err());
    }
}