...
async fn verify_rate_limit_averages() {
    ...
    assert!(
        rate >= 29.0 && rate <= 31.0,
        "Rate should be close to 30 req/s"
    );
    ...
    assert!(
        rate >= 42.0 && rate <= 46.0,
        "Rate should be around 42-45 req/s due to burst capacity"
    );
    ...
    println!(
        "  Per-second limiter: {} instant requests (should be 30)",
        burst_count
    );
    ...
    println!(
        "  15-second window: {} instant requests (should be 450)",
        burst_count
    );
    ...
    assert!(
        rate >= 34.0 && rate <= 38.0,
        "Rate should be around 35-37 req/s over 60s"
    );
