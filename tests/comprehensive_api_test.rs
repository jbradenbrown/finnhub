...
impl TestResults {
    ...
    fn print_summary(&self) {
        ...
        println!("Passed: {} ({:.1}%)", self.passed, (self.passed as f64 / self.total as f64) * 100.0);
        println!("Failed: {} ({:.1}%)", self.failed, (self.failed as f64 / self.total as f64) * 100.0);
...
async fn test_all_endpoints() {
    ...
    test_endpoint(&client, &mut results, "stock.candles", || async {
        ...
        client.stock().candles(stock_symbol, CandleResolution::Daily, from, to).await
    }).await;
    ...
    test_endpoint(&client, &mut results, "stock.financials", || async {
        ...
        client.stock().financials(stock_symbol, StatementType::IncomeStatement, StatementFrequency::Annual).await
    }).await;
    ...
    test_endpoint(&client, &mut results, "stock.eps_estimates", || async {
        client.stock().eps_estimates(stock_symbol, Some("quarterly")).await
    }).await;
    
    // Insider trading
    test_endpoint(&client, &mut results, "stock.insider_transactions", || async {
        client.stock().insider_transactions(stock_symbol).await
    }).await;
    
    test_endpoint(&client, &mut results, "stock.insider_sentiment", || async {
        let from = "2024-01-01";
        let to = "2024-12-31";
        client.stock().insider_sentiment(stock_symbol, from, to).await
    }).await;
    ...
    test_endpoint(&client, &mut results, "stock.social_sentiment", || async {
        let from = "2024-01-01";
        let to = "2024-01-31";
        client.stock().social_sentiment(stock_symbol, from, to).await
    }).await;
    
    test_endpoint(&client, &mut results, "stock.congressional_trading", || async {
        client.stock().congressional_trading(stock_symbol, None, None).await
    }).await;
    ...
    test_endpoint(&client, &mut results, "forex.candles", || async {
        ...
        let to = chrono::Utc::now().timestamp();
        client.forex().candles(forex_pair, CandleResolution::Daily, from, to).await
    }).await;
    ...
    test_endpoint(&client, &mut results, "crypto.candles", || async {
        ...
        let to = chrono::Utc::now().timestamp();
        client.crypto().candles(crypto_symbol, CandleResolution::SixtyMinutes, from, to).await
    }).await;
    ...
    test_endpoint(&client, &mut results, "etf.holdings", || async {
        client.etf().holdings(Some(etf_symbol), None, None, None).await
    }).await;
    ...
    test_endpoint(&client, &mut results, "scanner.support_resistance", || async {
        client.scanner().support_resistance(stock_symbol, "D").await
    }).await;
    
    test_endpoint(&client, &mut results, "scanner.aggregate_indicators", || async {
        client.scanner().aggregate_indicators(stock_symbol, "D").await
    }).await;
    ...
    test_endpoint(&client, &mut results, "misc.technical_indicator", || async {
        let from = chrono::Utc::now().timestamp() - 86400 * 30;
        let to = chrono::Utc::now().timestamp();
        let mut params = serde_json::Map::new();
        params.insert("timeperiod".to_string(), serde_json::json!(14));
        client.misc().technical_indicator(stock_symbol, "D", from, to, "sma", Some(serde_json::Value::Object(params))).await
    }).await;
    ...
    println!("\nTotal test duration: {:.2} seconds", duration.as_secs_f64());
    println!("Average time per endpoint: {:.2} seconds", duration.as_secs_f64() / results.total as f64);
