...
async fn test_rate_limiting_behavior() {
    ...
    println!("  Burst test result: {} succeeded, {} failed in {:?}", 
    ...
    while start.elapsed() < test_duration {
        ...
        match client.stock().quote("AAPL").await {
            Ok(_) => {
                ...
                println!("  Request {} succeeded at {:?}", 
                         total_requests + 1, start.elapsed());
            }
            Err(e) => {
                failed_requests += 1;
                println!("  Request {} failed at {:?}: {}", 
                         total_requests + 1, start.elapsed(), e);
            }
        }
    ...
    println!("  Sustained test result: {} total, {} succeeded, {} failed over {:?}",
             total_requests, successful_requests, failed_requests, start.elapsed());
    ...
    println!("  Exceed test result: {} succeeded, {} failed ({} rate limit errors) in {:?}",
             success_count, error_count, rate_limit_errors, start.elapsed());
    ...
    for i in 0..60 {
        match client.stock().quote("AAPL").await {
            Ok(_) => {
                success_count += 1;
                if i < 10 || i >= 50 {  // Only print first 10 and last 10
                    println!("  Request {} succeeded at {:?}", i + 1, start.elapsed());
                }
            }
            Err(e) => {
    ...
    println!("  Token bucket test result: {} succeeded, {} failed in {:?}",
             success_count, error_count, start.elapsed());
...
async fn test_internal_rate_limiter() {
    ...
    assert!(available >= 14 && available <= 16, "Should have refilled ~15 tokens");
